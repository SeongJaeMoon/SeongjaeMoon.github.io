...
저번 포스팅에 이어서 파이썬 기초부터 웹 크롤러 개발 두 번째 포스팅을 진행해보자. 이번 포스팅에선 코딩에 많은 부분을 차지하는 컬렉션과 함수에 대해서 알아보자. 테스트 코딩용 파이썬 에디터는 [Atom](https://atom.io/)과 script 패키지를 이용해서 작성하였고, [Python 3.6.4](https://www.python.org/downloads/) 버전을 사용했다. Java 코드 테스트는 마찬가지로 [Atom](https://atom.io/)과 JDK 8 환경에서 코딩되었다.
